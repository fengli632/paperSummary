...
<body>
				...
				<ul class="position-list">
					<c:forEach items="${loadByReportsUser.datas }" var="p">
						...
						<li>
							...
							<div class="position-name2" >${str}</div>
							<span class="position-type">${p.reportsUser.nickname }</span> 
							...
							<span class="position-time">
								<a href="<%=request.getContextPath() %>/reports/${p.id }/show">详情</a> &nbsp;
								<a href="<%=request.getContextPath() %>/reports/${p.id}/update">更新</a> &nbsp;
								<a href="<%=request.getContextPath() %>/reports/${p.id}/delete" onclick="return confirm('是否确认删除?')">删除</a> &nbsp;
							</span>
						</li>
	...
	<!--分页  -->
		<jsp:include page="/inc/pager.jsp">
			<jsp:param value="reportsList" name="url"/>
